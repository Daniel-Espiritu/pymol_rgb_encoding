...
def assign_ncolor(values, breaks, break_colors, return_dict=False):
    ...
    (break_colors) that are pre-assigned to given values (breaks). This
    function builds off of assign_color, but can be used for an n-color
    gradient, rather than a 2 color gradient.
